<template>
    <div class="app-container">
      <!-- 工具条 -->
      <div class="tools-div">
        <el-button type="success" icon="el-icon-plus" size="mini" @click="add()" :disabled="$hasBP('bnt.processTemplate.templateSet')  === false">添加審批設置</el-button></el-button>
      </div>
      <!-- 列表 -->
      <el-table
      ...
      >
        ...
        <el-table-column label="操作" width="250" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="show(scope.row)">查看審批設置</el-button>
            ...
            <el-button type="text" v-if="scope.row.status == 0" size="mini" @click="edit(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.templateSet')  === false">修改審批設置</el-button>
            <el-button type="text" v-if="scope.row.status == 0" size="mini" @click="removeDataById(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.remove')  === false">刪除</el-button>
            <el-button v-if="scope.row.status == 0" type="text" size="mini" @click="publish(scope.row.id)" :disabled="$hasBP('bnt.processTemplate.publish')  === false">發布</el-button></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
      ...
      <el-dialog title="查看審批設置" :visible.sync="formDialogVisible" width="35%">
        <h3>基本信息</h3>
        <el-divider/>
